...
def prep_mktdata(fulldata: list):
    ...
    for i in range(len(fulldata)):
        ...
        for j in range(len(mkt["contracts"])):
            ...
            data.append(
                (tstamp, id_mkt, id_contract, yes_bid, yes_ask, yes_mid, isopen)
            )
...
if __name__ == "__main__":
    ...
    # Upload data on disk to Postgres
    # This is run when the db connection has been failing, but we are still saving the data to disk
    ...
    execute_values(
        cur,
        "INSERT INTO map (id_mkt, id_contract, name_mkt, name_contract) VALUES %s ON CONFLICT (id_mkt, id_contract) DO NOTHING",
        map,
    )
    con.commit()
    execute_values(
        cur,
        "INSERT INTO data (tstamp, id_mkt, id_contract, yes_bid, yes_ask, yes_mid, isopen) VALUES %s",
        data,
    )
    con.commit()
